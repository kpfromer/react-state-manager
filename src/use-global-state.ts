...
type Action<S, A> = (state: S, action: A) => S;
...
function useCustom<S, R, A extends ActionObjectWithState<S, R, any>>(store: Store<S>, actions: A): () => [S, A] {
...
// function useCustom<T>(this: Store<T>) {
//   const newListener = React.useState()[1];
//   React.useEffect(() => {
...
function associateActions<S, A>(store: Store<S>, set: ReturnType<typeof setState>, actions: ActionObject<S, A>): ActionObjectWithState<S, A, any> {
  const associatedActions = {};
  for (let key of Object.keys(actions)) {
    if (typeof actions[key] === 'function') {
      // TODO: update for promise
      associatedActions[key] = (action: any) => set<S>(actions[key](store.state, action));
    }
...
const useGlobalHook = <S, A extends ActionObject<S, any>>(initialState: S, actions: A): () => [S, ActionObjectWithState<S, A, A>] => {
  const store: Store<S> = {
    state: initialState,
    listeners: []
  };
  ...
  return useCustom(store, associatedActions);
};
